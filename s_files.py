...
"""
#Writing from a file
f=open("s_myfile.txt","w")
string = input("Enter your comment here:")
f.write(string)
f.close()

#Reading from a file
f1=open("s_myfile.txt","r")
str1=f1.read(-1)
print(str1)
f1.close()
...
import os, sys
if os.path.isfile("s_mymultifile.txt"): #Checks if file exist
    f=open("s_mymultifile.txt","w")
...
f=open("s_serialized.dat","wb") #Creates file that serialized data would be stored
s=Student(1,"Nora", 85) #Data to be stored
pickle.dump(s,f) #Serializing the data
f.close()

r = open("s_serialized.dat","rb") #Read in serialized data
